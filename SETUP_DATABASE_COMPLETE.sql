-- =====================================================
-- COMPLETE DATABASE SETUP FOR BUDGET PLANNER
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE NOTICE 'Table categories does not exist. Creating all tables...';
    ...
    -- Drop existing trigger if exists
    DROP TRIGGER IF EXISTS check_budget_after_transaction ON transactions;
  ...
  ELSE
    RAISE NOTICE 'Table categories already exists. Skipping table creation.';
  END IF;
END $$;
...
SELECT
  ...
  EXISTS(SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'profiles') AS profiles,
  EXISTS(SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'categories') AS categories,
  EXISTS(SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'transactions') AS transactions,
  EXISTS(SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'budget_plans') AS budget_plans,
  EXISTS(SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'budget_alerts') AS budget_alerts;
